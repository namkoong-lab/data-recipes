...
class SimpleMixtureBenchmark():
    ...
    def func(self, z, x):
        ...
        if x_s in self.trained_models:
            ...
            m = sk_e.RandomForestRegressor(n_estimators = 0,
        ...
        for ds, this_x in zip(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'], x):
            this_data.append(self.data[ds].sample(int(len(self.data[ds])*this_x), random_state=self.seed))
        ...
        if m.n_estimators == z:
            ...
            return [[z, sk_m.mean_squared_error(self.data['I']['Z'], m.predict(self.data['I'][['X', 'Y']]))]]
        
        out = []
        while m.n_estimators < z:
            m.n_estimators += 1
            m.fit(X=this_data[['X', 'Y']], y=this_data['Z'])
            out.append([m.n_estimators, sk_m.mean_squared_error(self.data['I']['Z'], m.predict(self.data['I'][['X', 'Y']]))])
